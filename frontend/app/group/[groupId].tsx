...
export default function GroupChatScreen() {
  ...
  const fetchData = async () => {
    try {
      const [groupsData, messagesData] = await Promise.all([
        ...
        apiService.getGroupMessages(groupId as string),
      ]);
      const currentGroup = groupsData.groups?.find((g: any) => g.group_id === groupId);
  ...
  const sendMessage = async (content?: string, type: string = 'text', mediaData?: string) => {
    ...
    setIsSending(true);
    try {
      const response = await apiService.sendGroupMessage(groupId as string, {
        content,
        message_type: type,
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <View style={styles.headerIcon}>
            <Ionicons name="people" size={20} color="#6B7FD7" />
          </View>
          <View>
            <Text style={styles.headerName} numberOfLines={1}>{group?.name}</Text>
            <Text style={styles.headerMembers}>{group?.member_count} members</Text>
          </View>
        </View>
        <View style={{ width: 40 }} />
      </View>
